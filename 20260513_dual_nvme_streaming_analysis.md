...
## 8. 實現 Pipeline Overlap 的技術路徑

目前 pipeline overlap 未生效（Section 1.4）。要達到 Pipeline 模式的理論效能，需要：

### 8.1 方案 A：DirectStorage Async Read（推薦）

```
現行：
  std::ifstream.read(buffer, size)  ← 阻塞式讀取，佔用 CPU 線程

改為：
  IDStorageQueue->EnqueueRead(...)  ← 非阻塞 DMA，硬體直接 NVMe→USM
  // GPU kernel 在 DMA 期間繼續執行
  IDStorageQueue->GetStatus(fence)  ← 只在需要時等待完成
```

- 優點：真正的硬體 DMA overlap，不佔 CPU 或記憶體頻寬
- 缺點：需要 Windows DirectStorage SDK，較複雜的 API
- 參考：MoE OTD 已驗證此路徑可行

### 8.2 方案 B：背景線程 + Memory-Mapped IO

```
std::thread io_thread([&]{
    // 在背景線程中讀取下一個 group
    memcpy(buffer[next], mmap_ptr + offset, size);
    fence.signal();
});
// GPU 在主線程繼續計算
gpu_execute(current_group);
io_thread.join();
```

- 優點：實作簡單
- 缺點：memcpy 仍佔系統記憶體頻寬，可能與 iGPU 競爭

### 8.3 方案 C：使用 OpenVINO AsyncInferRequest 模式

- 讓 OpenVINO 的 async 機制處理 IO/compute overlap
- 需要重新設計 streaming 架構，與現有 `execute_impl_streamed()` 不相容
